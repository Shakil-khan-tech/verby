      ...
      <div class="row justify-content-center my-10 px-8 my-lg-15 px-lg-10">
        <div class="col-md-12">
          ...
          <div class="card card-custom gutter-b example example-compact">
            ...
            <!--begin::Form-->
            <form class="form" action="{{ route('records.update', $record->id) }}" method="post">
              @csrf
              {{ method_field('PUT') }}
              <div class="card-body">
                <div class="form-group">
                  <div class="alert alert-custom alert-default" role="alert">
                    ...
                    <div class="alert-text">{{ __('Edit the record here, which will then be synchronized with the device.') }}</div>
                  </div>
                </div>
                <div class="form-group row">
                  <div class="col-lg-6">
                    ...
                    <div class="input-group">
                      ...
                      <select class="form-control select2" id="employee_input" name="employee">
                  ...
                  <div class="col-lg-6">
                    ...
                    <div class="input-group">
                      ...
                      <select class="form-control" id="device_input" name="device">
                        <option value="{{ $record->device->id }}">{{ $record->device->name }}</option>
                      </select>
                    </div>
                    ...
                    <div class="input-group">
                      ...
                      <select class="form-control" id="action_input" name="action">
                        @foreach (Config::get('constants.actions') as $key => $action)
                          <option value="{{ $key }}" {{ $key == $record->action ? "selected" : "" }}>{{ $action }}</option>
                        @endforeach
                      </select>
                    </div>
                    ...
                    <div class="input-group">
                      ...
                      <select class="form-control" id="perform_input" name="perform">
                        @foreach (Config::get('constants.performs') as $key => $perform)
                          <option value="{{ $key }}" {{ $key == $record->perform ? "selected" : "" }}>{{ $perform }}</option>
                        @endforeach
                      </select>
                    </div>
                ...
                <div class="form-group row">
                  <div class="col-lg-6">
                    ...
                    <div class="input-group">
                      ...
                      <select class="form-control" id="identity_input" name="identity" disabled="disabled">
                        @foreach (Config::get('constants.identities') as $key => $identity)
                          <option value="{{ $key }}" {{ $key == $record->identity ? "selected" : "" }}>{{ $identity }}</option>
                        @endforeach
                      </select>
                    </div>
                  ...
                  <div class="col-lg-6">
                    ...
                    <div class="input-group date" id="record_datetime" data-target-input="nearest">
                      ...
                      <input type="text" name="time" class="form-control datetimepicker-input" placeholder="{{ __('Select date & time') }}"  data-toggle="datetimepicker" data-target="#record_datetime" value="{{ $record->time->format('d.m.Y H:i:s') }}" />
                    </div>
...
@section('scripts')
    ...
    var currentEmployee = {{ $record->employee->id }};
    var currentDepaRooms = {!! $record->rooms->where('pivot.clean_type', '=', 0)->where('pivot.status', '!=', 0) !!};
    var currentRestantRooms = {!! $record->rooms->where('pivot.clean_type', '=',  1)->where('pivot.status', '!=', 0) !!};
    </script>
