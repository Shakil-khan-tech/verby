        ...
        <div class="col-md-8">
          ...
          <div class="card card-custom gutter-b example example-compact">
            <div class="card-header">
              <h3 class="card-title">{{ __('Manage Room List issues') }}</h3>
            </div>
            <!--begin::Form-->
            <form class="form" action="{{ route('issues.store') }}" method="post">
              @csrf
              <div class="card-body">
                <div id="issues_repeater">

                  <div data-repeater-list="issues" class="form-group row">
                    @forelse($issues as $issue)
                      <div data-repeater-item="" class="form-group row align-items-center col-lg-12">
                        <div class="col-md-6">
                          <label>{{ __('Name') }}:</label>
                          <input type="text" name="name" class="form-control" placeholder="{{ __('Enter issue name') }}" value="{{ $issue->name }}" required />
                    ...
                    @empty
                      <div data-repeater-item="" class="form-group row align-items-center col-lg-12">
                        <div class="col-md-6">
                          <label>{{ __('Name') }}:</label>
                          <input type="text" name="name" class="form-control" placeholder="{{ __('Enter issue name') }}" required />
                          <div class="d-md-none mb-2"></div>
                        </div>
                        <div class="col-md-2">
                          <label>{{ __('Action') }}:</label><br>
              ...
              <div class="card-footer">
                ...
                <a class="btn btn-secondary" href="{{ route('issues.listings') }}">{{ __('Cancel') }}</a>
              </div>
            </form>
            <!--end::Form-->
          </div>
...
@section('scripts')
    ...
    {{-- page scripts --}}
    <script src="{{ mix('js/pages/issues/issues.js') }}" type="text/javascript"></script>
