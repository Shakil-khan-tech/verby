...
<html lang="{{ app()->getLocale() }}">
  ...
  <body>
    <h3 class="text-center">{{ __('Active Issues') }}</h3>
    ...
    <table id="issuesTable" class="table bg-white">
      <thead>
      ...
      <tbody>
        ...
        <tr class="table-secondary text-center">
          <td scope="row">{{ $listing->issue->name }}</td>
          <td scope="row">{{ $listing->room->device->name }}</td>
          <td scope="row">{{ $listing->room->name }}</td>
          <td scope="row">{{ $listing->date_requested }}</td>
          <td scope="row">{{ $listing->userRequested->name }}</td>
          <td scope="row">{{ $listing->comment_requested }}</td>
          <td scope="row">{{ isset( Config::get('constants.priorities')[$listing->priority] ) ? Config::get('constants.priorities')[$listing->priority] : '' }}</td>
        </tr>
        @empty
        <tr class="table-secondary text-center">
