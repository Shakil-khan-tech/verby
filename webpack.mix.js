...
// mix.copyDirectory('resources/tarifa', 'public/tarifa')

//custom
mix.js('resources/js/custom.js', 'public/js').version();
mix.css('resources/css/print_calendar.css', 'public/css/print');

// Custom js pages (single page use)
(glob.sync('resources/js/pages/**/*.js') || []).forEach(file => {
    mix.js(file, `public/${file.replace('resources/', '')}`).version();
});

// Fixes for Metronic
mix.copy('resources/js/fix/', 'public/js/fix/');

// Raw plugins - doing this for fullcalendar
